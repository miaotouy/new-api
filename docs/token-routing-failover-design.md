...
> 状态说明：✅ 已实现并纳入当前代码；⚠️ 已实现主链路，但专项集成/跨数据库/前端交互测试仍待补齐。

| 章节 | 状态 | 当前施工结果 |
|---|---|---|
| 2. 密钥配置与接口 | ✅ | `Token` 新字段、旧 `cross_group_retry` 兼容、`TokenRouteRule` 模型、GORM 迁移、规则读写 API、Token 删除级联清理均已实现。 |
| 3. 路由计划服务 | ✅ | 已生成按候选分组/渠道的 RoutePlan；手动规则保持规则顺序，组规则按 priority + weight 选择；自动 price 模式按预计成本、priority、响应时间、weight、渠道 ID 排序。 |
| 4. 重试与故障转移 | ✅ | RoutePlan 记录已消费渠道，跳过禁用/不可用渠道后不会重复命中；故障原因、候选分组、候选渠道、尝试序号和最大倍率记录至 `other.admin_info.route_attempts`。 |
| 5. 计费与额度安全 | ✅ | 首次上游调用前遍历所有合规候选分组，按其中最大预扣额度预扣；每次实际选路都会同步最终分组到 `RelayInfo` 并按实际分组结算。 |
| 6. Key 级速率限制 | ✅ | 已在 Token 认证后、渠道分配前接入；Redis 使用 Lua 原子固定窗口计数，异常时回退内存限流，限流键仅使用 Token ID。 |
| 7. 管理端界面 | ✅ | 已提供自动/手动模式、价格优先、倍率、故障转移、限流、规则编辑器和拖拽排序；已展示渠道状态、分组倍率与支持模型。 |
| 8. 测试计划 | ⚠️ | 已补充路由权重选择、去重、price tie-break 和路由审计的单元回归；`go test ./service ./controller ./middleware ./model` 与 `bun run typecheck` 已通过。SQLite/MySQL/PostgreSQL 迁移、Redis 集成限流和前端交互测试仍应按本节继续补齐。 |
